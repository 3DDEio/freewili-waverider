...
| Pocket Alert haptics | With Pocket Alert enabled, a below-to-above threshold crossing produces exactly three pulses; a steady carrier does not repeat, another crossing inside 30 seconds is suppressed, and a crossing after both cooldown and 3 dB re-arm alerts again. Settings survive restart. | Read-only copies of the installed Doom and Meshtastic UF2s were checksummed and inspected. Meshtastic embeds its public FreeWili source origin; that branch maps HAP_MOTOR to Display GPIO46, active-high at 12 mA, with three 150 ms pulses and 80 ms gaps. Doom independently contains a dedicated PWM haptic driver. WaveRider now matches that shipped implementation. See `docs/HAPTIC_EVIDENCE.md`. | **Pending physical Test, RF crossing, cooldown, re-arm, and persistence validation** |
