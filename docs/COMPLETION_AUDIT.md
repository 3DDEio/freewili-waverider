...
| Reproducible public deployment | Clean release contains the final CM0/native artifacts, documentation, offline dependencies, corresponding source, checksums, and a safe install/rollback path; every linked dependency permits public redistribution. | The 2026-08-12 release audit pinned WiliBSP and nested OneWili, stored reviewed compatibility patches, added exact Debian rtl-sdr corresponding source, pinned current GitHub Actions, restricted the CM0 transfer manifest, and made the native installer stage/read back/promote/restore across interrupted upgrades. Cross-host validation found that Arm's macOS and Linux 14.2.Rel1 distributions order several newlib objects differently, so the pinned GitHub Linux x86_64 job is the declared canonical builder. The current canonical Display and installer UF2 SHA-256 values are `2a095c746b594ae94d77bb624cd22d64237913df20b7cc630ca8bd9ab07aa892` and `7df3cc2d6c6b72832ff3737828cf590c414b97af7bea10d638c611524c948358`; two clean hosted Linux builds compared every generated UF2/ELF product byte-for-byte. Generated products are not committed. Repeated device and WaveRider project-source release builds are byte-identical with exact digests in generated sidecars. Both builders use tracked input only and one portable deterministic writer. The project-source archive excludes WiliBSP and OneWili, records the reviewed upstream commits, and provides a helper that fetches those exact revisions directly from FreeWili. Clean extraction passed checksums, corresponding-source verification, sensitive-file exclusion, and an SRAM-only dry run. The final CM0 installer also restores its complete predecessor on an interrupt during integration. On connected FW2 v07 hardware the native installer first failed closed while Main reported SD state `none`/SDFS `NO_CARD`, leaving the existing app intact. A normal vendor software launch remounted the card as `main`; the migration installer then staged and byte-verified 204,288 bytes, retained the previous UF2 at `/appdata/waverider/waverider_display.previous.uf2`, promoted `/apps/Radio/WaveRider.uf2`, removed `/apps/Radio/waverider_display.uf2`, and removed `/apps/waverider` only after listing it and proving it empty. Main subsequently launched the friendly-name path successfully. The final About observation remains pending. More importantly, the pinned public OneWili repository contains no explicit license, so WaveRider may not claim or publish a supported linked binary release until FreeWili grants redistribution permission. | **STOP SHIP: OneWili permission plus About validation required** |
