...
| Morse message detection | A clean, centered, repeating NFM Morse transmission is decoded without stopping the waterfall; only a message supported by at least three agreeing receptions produces a bounded popup; verified detections remain browsable by frequency with time/repeats/quality; confirmed Clear removes all history; and an unmodulated carrier or inconsistent callsign is withheld. | Continuous IQ capture is now measured at 0.990--0.999x real time on the CM0. Three controlled 147.500 MHz receptions decoded the exact ground truth `KO6FQY JOIN NORCALCYBER.IO! KO6FQY`; persistent history contains one verified 3/3 record with agreement 1.00, and physical recall of that exact record from MSGS is confirmed. Its payload now uses the larger native font with word wrapping. The first exact reception retained all 108 marks/107 gaps over 37.36 seconds at 93.3 ms / 13 WPM, 0.964 overall confidence, and 0.928 timing confidence. Earlier rejected variants remained unverified. The ten-transmission efficacy sample, equal-duration 144.300 MHz control, and physical MSGS grouping/Clear checks remain. | **Core over-air decode, persistence, and safe consensus proven; full field gate still pending** |
